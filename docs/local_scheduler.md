...
전체 공개 소스 기반 신규 종목/일정/파생 데이터 갱신:
...
- `baseline`: weekdays at `17:30`, only when `IPO_DATA_BASELINE_ENABLED=1`.
  Defaults to `public-refresh`, which attempts Finuts plus configured public
  sources such as DART/iTick/IPOKorea/articles/Naver.

For Finuts-free local operation, keep `IPO_DATA_LIVE_BATCH=naver-live`,
`IPO_DATA_BASELINE_BATCH=public-refresh-no-finuts`,
`IPO_DATA_DEMAND_ENABLED=0`, and `IPO_DATA_BASELINE_ENABLED=1`.
